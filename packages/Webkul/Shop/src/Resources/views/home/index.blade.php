...
<x-shop::layouts>
    ...
    @foreach ($customizations as $customization)
        ...
        @if ($customization->sort_order >= 2)
            @once
                @include('shop::home.home_slider')
            @endonce
        @endif
        ...
        @if ($customization->sort_order >= 5)
            @once
                @include('shop::home.banner')
            @endonce
        @endif

        @if ($customization->sort_order >= 6)
            @once
                @include('shop::home.auction2')
            @endonce
        @endif

        @if ($customization->sort_order >= 7)
            @once
                @include('shop::home.comment')
            @endonce
        @endif

        @if ($customization->sort_order >= 8)
            @once
                @include('shop::home.weekly_features')
            @endonce
        @endif

        @if ($customization->sort_order >= 8)
            @once
                @include('shop::home.news')
            @endonce
        @endif
